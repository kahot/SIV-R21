...
for (j in 1:length(monkeyList)){
    ...
    summ1<-sample[,c("Monkey","Tissue3","Week","Cohort","filename")]
    summ1$sample<-paste0("Week",summ1$Week,"_",summ1$Tissue3)
...
aa<-aa[,c("Monkey","Tissue3","Group","AA112")]
s<-data.frame(Monkey=rep("Stock", times=2), Tissue3=rep("Stock", times=2),
              Group=c("Sustained AA sub","No sweep"),
              AA112=f)
aa<-rbind(aa,s)
aa$Tissue3<-factor(aa$Tissue3, levels=c("Stock","Plasma","Plasma nex", "Peripheral LN","Thoracic LN", "Lung"))
...
ggplot(aa, aes(x=Tissue3, y=AA112, color=Monkey))+
    geom_point(position=position_dodge(width = 0.3))+
    scale_y_continuous(breaks=c(0,0.25,0.5,0.75,1), limits=c(0,1))+
    theme_bw()+xlab('')+ylab("Freq. of Adenosine at AA112 (nt335)")+
    theme(axis.text.x = element_text(angle =45, hjust=1))+
