...
csns<-read.csv("Output/Consensus_merged.pos.ita.csv", stringsAsFactors = F, row.names = 1)
...
for (i in 1:length(SIVFiles)){
    ...
    for (k in 1:nrow(SeqData)){
        if (is.na(SeqData$MajNt[k])) {
            ...
            Tvs1rNum<-SeqData[k,paste0(transv1(SeqData$Ita.stock[k]))]
            Tvs2rNum<-SeqData[k,paste0(transv2(SeqData$Ita.stock[k]))]
